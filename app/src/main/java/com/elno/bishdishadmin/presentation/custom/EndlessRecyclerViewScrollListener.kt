package com.elno.bishdishadmin.presentation.custom
...
abstract class EndlessRecyclerViewScrollListener(visibleThresholdParam: Int = 4) : RecyclerView.OnScrollListener() {
	// The minimum amount of items to have below your current scroll position
	// before loading more.
	private var visibleThreshold = visibleThresholdParam
	...
	override fun onScrolled(view: RecyclerView, dx: Int, dy: Int) {
		...
		// If isn't still loading, we check to see if the dataset count has
		// changed, if so we conclude it has finished loading and update the current page
		// number and total item count.
